...
(() => {
  ...
  const ART = {
    welcome: './auth-art/welcome-crest.jpg',
    book: './auth-art/login-book.jpg',
    people: './auth-art/signup-people.jpg',
    lock: './auth-art/otp-lock.jpg',
    shield: './auth-art/success-shield.jpg'
  };
  ...
  const passField = (id, ph) => `<div class="ah-eye"><input class="ah-inp" id="${id}" type="password" placeholder="${ph}" autocomplete="new-password"><button type="button" data-eye="${id}" aria-label="Show password">👁</button></div>`;

  const welcome = () => paint(`<section class="ah-welcome">
    <img src="${ART.welcome}" alt="Admission Hub">
    <h1>ADMISSION HUB</h1>
  ...
  const login = (tab) => {
    tab = tab || 'pass';
    paint(`<div class="ah-phone">
      <div class="ah-hero"><img src="${ART.book}" alt=""></div>
      ...
      <div id="ahGoogleSlot"><button class="ah-btn sec" type="button" id="ahGoogle">G  Continue with Google</button></div>
      <div class="ah-foot">Don't have an account? <button class="ah-link" type="button" data-go="signup">Sign Up</button></div>
    </div>`);
    mountGoogle();
  };

  const signup = () => paint(`<div class="ah-phone">
    <button class="ah-back" type="button" data-go="login">←</button>
    <div class="ah-hero"><img src="${ART.people}" alt=""></div>
  ...
  const otpScreen = () => paint(`<div class="ah-phone">
    <button class="ah-back" type="button" data-go="${draft.purpose === 'reset' ? 'forgot' : (draft.purpose === 'signup' ? 'signup' : 'login')}">←</button>
    <div class="ah-hero"><img src="${ART.lock}" alt=""></div>
    <div class="ah-kicker">Verify Your<br>Number</div>
    <p class="ah-sub">We sent a 6-digit code to<br><b>${esc(draft.masked || draft.id)}</b><br><button class="ah-link" type="button" data-go="${draft.purpose === 'signup' ? 'signup' : 'login'}">Change</button></p>
    ...
    <div class="ah-foot" id="ahOtpWait">Resend code in <b id="ahSec">${draft.wait || 45}</b></div>
  ...
  const success = (title, sub, go) => paint(`<div class="ah-phone ah-success">
    <img src="${ART.shield}" alt="">
    <div class="ah-kicker" style="margin-top:8px">${title}</div>
    <p class="ah-sub">${sub}</p>
    <button class="ah-btn" type="button" id="ahEnter">${go}</button>
  </div>`);

  const forgot = () => paint(`<div class="ah-phone">
    <button class="ah-back" type="button" data-go="login">←</button>
    <div class="ah-hero"><img src="${ART.lock}" alt=""></div>
  ...
  const reset = () => paint(`<div class="ah-phone">
